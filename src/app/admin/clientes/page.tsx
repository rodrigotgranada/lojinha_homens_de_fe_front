...
export default function GestaoClientesPage() {
  ...
  if (isLoading || loading) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center py-20 gap-3">
        ...
        <span className="text-zinc-505 font-semibold text-sm">Carregando gerenciamento de contatos...</span>
      </div>
    );
  }
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-800 rounded-3xl p-8 max-w-md w-full text-center space-y-6 shadow-xl">
          ...
          <div className="space-y-2">
            ...
            <p className="text-zinc-500 dark:text-zinc-400 text-sm">
              Esta área de gestão de clientes e históricos fiscais é reservada exclusivamente para administradores.
            </p>
          </div>
          <Link
            href="/login"
  ...
  return (
    <CustomerProvider value={customerState}>
      <div className="flex-1 flex flex-col gap-6 py-6 max-w-7xl mx-auto w-full px-4">
        {/* Header */}
        <CustomerHeader />

        {error && (
          <div className="bg-rose-50 border border-rose-200 text-rose-700 px-4 py-3.5 rounded-2xl flex items-center gap-2 text-sm shadow-xs">
        ...
        <div className="flex border-b border-zinc-200 dark:border-zinc-800 gap-6 text-sm mb-6 mt-2">
          ...
          >
            Ativos ({customerState.customers.filter((c) => c.active !== false).length})
          </button>
          <button
            onClick={() => customerState.setActiveTab("inactive")}
            className={`pb-3 font-bold cursor-pointer transition-all border-b-2 -mb-[2px] ${
              customerState.activeTab === "inactive"
                ? "border-indigo-600 text-indigo-650 dark:text-indigo-400"
                : "border-transparent text-zinc-400 hover:text-zinc-650 dark:hover:text-zinc-300"
            }`}
          >
            Inativos ({customerState.customers.filter((c) => c.active === false).length})
          </button>
        </div>

        {/* Clients Table (Search bar is inside) */}
        <CustomerTable />

        {/* Collapsible Purchase History Modal */}
